...
public abstract class Cajero {
    ...
    public Ticket consultarSaldo(String numeroCuenta){
        Ticket ticket = null;
        Cuenta cuenta = (Cuenta) this.buscar(numeroCuenta)[0]; //Castear> Deduce y lo cambia a tipo cuenta
        if (cuenta!=null) {
            System.out.println("Tu saldo es: " + cuenta.getSaldo());
            ticket = new Ticket(this.ubicacion, LocalDate.now(), "CONSULTA SALDO",cuenta.getSaldo(), (int) (Math.random()*100));
        }
        return ticket;
    }
